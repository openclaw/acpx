...
| `--model <id>`                           | Set agent model                                | Claude-compatible adapters may consume session creation metadata; startup-flag adapters receive a launch `--model` flag; other agents must advertise a model config option or legacy `models` metadata. |
